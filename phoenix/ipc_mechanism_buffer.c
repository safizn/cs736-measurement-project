...
void init_ipc_throughput_test(struct ipc_throughput_test **test) {
    ...
    {
        char *parent_write_buffer = malloc(SIZE);
        for (int i = 0; i < SIZE; i++)
            parent_write_buffer[i] = 'a';

        *test = malloc(sizeof(struct ipc_throughput_test));
        (*test)->memptr = memptr;
        (*test)->parent_write_buffer = parent_write_buffer;
        (*test)->semptr = semptr;
...
void run_ipc_throughput_test(struct ipc_throughput_test *test) {
    memcpy(test->memptr, test->parent_write_buffer, SIZE);
    sem_post(test->semptr);
}

void free_ipc_throughput_test(struct ipc_throughput_test *test) {
    wait(NULL);
    munmap(test->memptr, SIZE);
    sem_close(test->semptr);
    free(test->parent_write_buffer);
    free(test);
}
